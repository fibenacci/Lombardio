create table if not exists customer.customer_portal_credentials (
...
create table if not exists customer.customer_portal_invitations (
...
create index if not exists idx_customer_portal_invitations_customer
    on customer.customer_portal_invitations (customer_id, issued_at desc);

create table if not exists customer.customer_portal_sessions (
    token varchar(120) primary key,
    customer_id varchar(64) not null,
    tenant_id varchar(64) not null,
    issued_at timestamp with time zone not null,
...
create index if not exists idx_customer_portal_sessions_customer
    on customer.customer_portal_sessions (customer_id, issued_at desc);
