...
public class AuctionService {
    ...
    public Auction announceAuction(String tenantId, String auctionId, AnnounceAuctionCommand request) {
        ...
        if (request.auctionDate().isBefore(announcementDate.plusDays(7))
            ...
            throw new IllegalArgumentException("auctionDate must be between 7 and 14 days after public announcement");
        }
    ...
    public Auction openAuction(String tenantId, String auctionId) {
        ...
        List<AuctionLot> lots = auction.lots().stream()
                .map(lot -> new AuctionLot(
                        lot.id(), lot.auctionId(), lot.lotNumber(), lot.contractNumber(), lot.itemNumber(),
                        lot.description(), lot.estimatedValue(), lot.outstandingClaim(), lot.latestBidAmount(),
                        lot.leadingBidder(), lot.hammerPrice(), AuctionLotStatus.OPEN, lot.surplusAmount(),
                        lot.authorityTransferDueDate(), lot.authorityTransferStatus()
                ))
                .toList();
        Auction updated = new Auction(
                auction.id(), auction.tenantId(), auction.title(), auction.location(), AuctionStatus.LIVE,
                auction.publicAnnouncementDate(), auction.auctionDate(), Instant.now(clock), auction.closedAt(),
                auction.announcementReference(), lots, auction.createdAt(), Instant.now(clock)
        );
        return auctionRepository.save(updated);
    }
    ...
    public Auction closeAuction(String tenantId, String auctionId) {
        Auction auction = requireAuction(tenantId, auctionId);
        Auction updated = new Auction(
                auction.id(), auction.tenantId(), auction.title(), auction.location(), AuctionStatus.CLOSED,
                auction.publicAnnouncementDate(), auction.auctionDate(), auction.liveStartedAt(), Instant.now(clock),
                auction.announcementReference(), auction.lots(),
                auction.createdAt(), Instant.now(clock)
        );
        return auctionRepository.save(updated);
    }
    ...
    public Auction placeBid(
        ...
        List<AuctionLot> lots = auction.lots().stream()
                .map(lot -> updateBid(lot, lotId, request))
                .toList();
        Auction updated = new Auction(
                auction.id(), auction.tenantId(), auction.title(), auction.location(), auction.status(),
                auction.publicAnnouncementDate(), auction.auctionDate(), auction.liveStartedAt(), auction.closedAt(),
                auction.announcementReference(), lots, auction.createdAt(), Instant.now(clock)
        );
        return auctionRepository.save(updated);
    }

    public Auction settleLot(String tenantId, String auctionId, String lotId, SettleAuctionLotCommand request) {
        Auction auction = requireAuction(tenantId, auctionId);
        List<AuctionLot> lots = auction.lots().stream()
                .map(lot -> settle(lot, lotId, request.hammerPrice()))
                .toList();
        Auction updated = new Auction(
                auction.id(), auction.tenantId(), auction.title(), auction.location(), auction.status(),
                auction.publicAnnouncementDate(), auction.auctionDate(), auction.liveStartedAt(), auction.closedAt(),
    ...
    public List<SurplusCase> listSurplusCases(String tenantId) {
        return auctionRepository.findByTenantId(tenantId).stream()
                ...
                .filter(lot -> lot.surplusAmount() != null && lot.surplusAmount().compareTo(BigDecimal.ZERO) > 0)
                .map(lot -> new SurplusCase(
                        lot.auctionId(),
                        lot.id(),
                        lot.lotNumber(),
                        lot.contractNumber(),
    ...
    private AuctionLot updateBid(AuctionLot lot, String lotId, PlaceBidCommand request) {
        ...
        return new AuctionLot(
                lot.id(), lot.auctionId(), lot.lotNumber(), lot.contractNumber(), lot.itemNumber(),
                lot.description(), lot.estimatedValue(), lot.outstandingClaim(), request.amount(),
                request.bidderDisplayName(), lot.hammerPrice(), AuctionLotStatus.OPEN, lot.surplusAmount(),
                lot.authorityTransferDueDate(), lot.authorityTransferStatus()
        );
    }
    ...
    private AuctionLot settle(AuctionLot lot, String lotId, BigDecimal hammerPrice) {
        ...
        LocalDate transferDueDate = surplus.compareTo(BigDecimal.ZERO) > 0
                ? LocalDate.of(LocalDate.now(clock).getYear(), Month.DECEMBER, 31).plusYears(3).plusMonths(1)
                : null;
        return new AuctionLot(
                lot.id(), lot.auctionId(), lot.lotNumber(), lot.contractNumber(), lot.itemNumber(),
                lot.description(), lot.estimatedValue(), lot.outstandingClaim(), lot.latestBidAmount(),
                lot.leadingBidder(), hammerPrice, hammerPrice.compareTo(BigDecimal.ZERO) > 0 ? AuctionLotStatus.SOLD : AuctionLotStatus.UNSOLD,
                surplus, transferDueDate, surplus.compareTo(BigDecimal.ZERO) > 0 ? "OPEN" : "NOT_APPLICABLE"
        );
    }
