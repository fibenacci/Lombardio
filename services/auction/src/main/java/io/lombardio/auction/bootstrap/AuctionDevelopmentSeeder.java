...
public class AuctionDevelopmentSeeder {
  ...
  AuctionDevelopmentSeeder(
      AuctionRepository auctionRepository, DemoDataProperties demoDataProperties) {
  ...
  private Auction buildAuction(DemoTenant tenant, int tenantIndex, int auctionIndex) {
    ...
    Instant liveStartedAt =
        status == AuctionStatus.LIVE
            ...
            : null;
    Instant closedAt =
        status == AuctionStatus.CLOSED || status == AuctionStatus.SETTLED
  ...
  private List<AuctionLot> buildLots(
      DemoTenant tenant, int auctionIndex, AuctionStatus auctionStatus) {
    int lotCount = 6 + (auctionIndex % 4);
    List<AuctionLot> lots = new ArrayList<>();
    for (int lotIndex = 1; lotIndex <= lotCount; lotIndex++) {
      BigDecimal estimatedValue =
          new BigDecimal(120 + (lotIndex * 45L) + (auctionIndex * 10L) + ".00");
      BigDecimal outstandingClaim =
          estimatedValue
              .multiply(new BigDecimal("0.65"))
              .setScale(2, java.math.RoundingMode.HALF_UP);
      AuctionLotStatus lotStatus =
          switch (auctionStatus) {
            case DRAFT, ANNOUNCED -> AuctionLotStatus.PENDING;
            case LIVE -> lotIndex % 3 == 0 ? AuctionLotStatus.SOLD : AuctionLotStatus.OPEN;
            case CLOSED, SETTLED ->
                lotIndex % 4 == 0 ? AuctionLotStatus.UNSOLD : AuctionLotStatus.SOLD;
          };
      BigDecimal latestBidAmount =
          lotStatus == AuctionLotStatus.PENDING
              ? BigDecimal.ZERO
              : outstandingClaim.add(new BigDecimal(20 + lotIndex * 5L + ".00"));
      BigDecimal hammerPrice =
          lotStatus == AuctionLotStatus.SOLD ? latestBidAmount.add(new BigDecimal("15.00")) : null;
      BigDecimal surplusAmount =
          hammerPrice == null ? null : hammerPrice.subtract(outstandingClaim).max(BigDecimal.ZERO);
      ...
      lots.add(
          new AuctionLot(
              ...
              "VT-"
                  + tenant.numberPrefix()
                  + "-"
                  + String.format("%05d", auctionIndex * 10 + lotIndex),
              "ITEM-"
                  + tenant.numberPrefix()
                  + "-"
                  + String.format("%05d", auctionIndex * 10 + lotIndex),
              ...
              outstandingClaim,
              latestBidAmount,
              lotStatus == AuctionLotStatus.SOLD || lotStatus == AuctionLotStatus.OPEN
                  ? "Bieter " + lotIndex
                  : null,
              hammerPrice,
              lotStatus,
