...
class AuctionServiceTest {
    ...
    void createsAuctionAndCalculatesSurplusDueDate() {
        var response = service.createAuction("tenant-default", new CreateAuctionCommand(
                "Fruehjahrsauktion",
                "Berlin",
                List.of(new CreateAuctionLotCommand(
                        "PS-5001",
                        "PS-5001-01",
                        "Goldring 585",
        ...
        var announced = service.announceAuction(
                "tenant-default",
                response.id(),
        ...
        var afterBid = service.placeBid(
                "tenant-default",
                live.id(),
                live.lots().get(0).id(),
                new PlaceBidCommand("Bieter 17", new BigDecimal("620.00"))
        );
        var settled = service.settleLot(
                "tenant-default",
                afterBid.id(),
        ...
        assertThat(settled.lots().get(0).authorityTransferDueDate()).isEqualTo(LocalDate.of(2029, 12, 31).plusMonths(1));
    }
    ...
    private static final class InMemoryAuctionRepository implements AuctionRepository {
        ...
        public Optional<Auction> findByTenantIdAndId(String tenantId, String auctionId) {
            return auctions.stream().filter(auction -> auction.tenantId().equals(tenantId) && auction.id().equals(auctionId)).findFirst();
        }
