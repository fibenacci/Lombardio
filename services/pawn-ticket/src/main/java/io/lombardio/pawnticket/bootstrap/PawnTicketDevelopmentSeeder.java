...
public class PawnTicketDevelopmentSeeder {
  ...
  private PawnTicket buildTicket(DemoTenant tenant, int tenantIndex, int ticketIndex) {
    ...
    Instant createdAt =
        Instant.now().minusSeconds((long) (ticketIndex + tenantIndex * 5) * 86_400L);
    ...
    BigDecimal loanAmount =
        positions.stream()
            .map(PawnTicketPosition::pledgedValue)
            .reduce(BigDecimal.ZERO, BigDecimal::add)
            .multiply(new BigDecimal("0.72"))
            .setScale(2, RoundingMode.HALF_UP);
    BigDecimal monthlyInterestRate = new BigDecimal("0.035");
    BigDecimal monthlyOperatingFee =
        ticketIndex % 6 == 0 ? new BigDecimal("22.50") : new BigDecimal("15.00");
    BigDecimal totalInterestAmount =
        loanAmount.multiply(monthlyInterestRate).setScale(2, RoundingMode.HALF_UP);
    BigDecimal totalOperatingFeeAmount =
        monthlyOperatingFee.multiply(BigDecimal.valueOf(4L)).setScale(2, RoundingMode.HALF_UP);
  ...
  private List<CashTransaction> buildTransactions(
      PawnTicket ticket, DemoTenant tenant, int ticketIndex) {
