...
public class PawnTicketDocumentService {
    ...
    public byte[] render(PawnTicket pawnTicket) {
        try {
            ...
            document.add(keyValueTable(
                    new String[][]{
                            {"Ausstellungsdatum", DATE_FORMAT.format(pawnTicket.createdAt().atZone(ZoneOffset.UTC).toLocalDate())},
                            ...
                            {"Monatszins", pawnTicket.monthlyInterestRate().stripTrailingZeros().toPlainString() + " %"},
            ...
            signature.add(new Chunk("Unterschrift Kundin/Kunde: _______________________________\n\n", BODY_FONT));
            signature.add(new Chunk("Unterschrift Pfandleiher/in: _______________________________", BODY_FONT));
    ...
    public byte[] renderLabels(PawnTicket pawnTicket) {
        try {
            ...
            for (int index = 0; index < pawnTicket.positions().size(); index++) {
                ...
                document.add(new Paragraph("Beleihwert " + formatCurrency(position.pledgedValue()), BODY_FONT));
            }
