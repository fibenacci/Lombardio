...
class CashTransactionServiceTest {
    ...
    private final InMemoryRepositories.PawnTickets pawnTicketRepository = new InMemoryRepositories.PawnTickets();
    private final PawnTicketPolicyService pawnTicketPolicyService = new PawnTicketPolicyService(
            pawnTicketRepository,
            new PawnTicketTermsService(),
            Clock.fixed(Instant.parse("2026-03-18T12:00:00Z"), ZoneOffset.UTC)
    );
    private final CashTransactionService cashTransactionService = new CashTransactionService(
            pawnTicketRepository,
    ...
    void shouldPersistRedeemTransactionInJournal() {
        pawnTicketPolicyService.issue(new IssuePawnTicketCommand(
                "tenant-default",
                "customer-1",
                "KD-1001",
                "Anna Becker",
                "+49 170 111111",
                List.of(new PawnTicketPosition(null, null, "Goldring 585", "Gelbgold 14 Karat", new BigDecimal("180.00"))),
                new BigDecimal("180.00"),
                3,
                null
        ));

        var transaction = cashTransactionService.execute(new ExecuteCashTransactionCommand(
                "tenant-default",
                "PS-5001",
