...
public final class PlatformSeedFixtures {
  ...
  private static final List<DemoTenant> DEMO_TENANTS =
      List.of(
          new DemoTenant(
              "tenant-default",
              "default",
  ...
  public static List<DemoTenant> tenantsForScale(String scale) {
    int count =
        switch (scale == null ? "medium" : scale.trim()) {
          case "small" -> 2;
  ...
  public static Tenant toTenant(DemoTenant tenant, Instant timestamp) {
    return new Tenant(
        tenant.id(), tenant.key(), tenant.displayName(), tenant.status(), timestamp, timestamp);
  }
  ...
  public static List<TenantFeature> tenantFeatures(DemoTenant tenant, Instant timestamp) {
    ...
    features.add(
        new TenantFeature(
            tenant.id(), "customer-management", tenant.customerManagement(), timestamp));
    features.add(
        new TenantFeature(
            tenant.id(), "collateral-management", tenant.collateralManagement(), timestamp));
    features.add(
        new TenantFeature(tenant.id(), "aml-compliance", tenant.amlCompliance(), timestamp));
    features.add(
        new TenantFeature(
            tenant.id(), "kyc-provider-verification", tenant.kycProviderVerification(), timestamp));
    features.add(
        new TenantFeature(tenant.id(), "kyc-document-ocr", tenant.kycDocumentOcr(), timestamp));
    features.add(
        new TenantFeature(tenant.id(), "auction-workflow", tenant.auctionWorkflow(), timestamp));
    features.add(
        new TenantFeature(tenant.id(), "online-auctions", tenant.onlineAuctions(), timestamp));
    return features;
  }
