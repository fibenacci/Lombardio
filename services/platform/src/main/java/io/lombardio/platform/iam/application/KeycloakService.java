...
public class KeycloakService {
    ...
    public String createTenantGroup(String tenantId, String displayName) {
        ...
        if (response.getStatus() != 201) {
             throw new RuntimeException("Failed to create Keycloak group for tenant: " + response.getStatus());
        }
    ...
    public String createTenantUser(String tenantId, String email, String password, List<String> roles) {
        ...
        // Assign to group
        GroupRepresentation group = realmResource.groups().groups(tenantId, 0, 1).stream()
    ...
    public List<String> getAvailableRoles() {
        ...
        return realmResource.roles().list().stream()
                ...
                .filter(name -> !name.startsWith("default-roles-")) // Filter out Keycloak default roles if desired
                .toList();
    }
}
