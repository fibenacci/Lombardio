...
import io.lombardio.aml.api.http.AmlStatusResponse;
...
public class AmlService {
    ...
    public AmlStatusResponse getStatus(String tenantId, String customerId) {
        AmlCase amlCase = amlRepository.findByTenantIdAndCustomerId(tenantId, customerId)
    ...
    public AmlStatusResponse updateStatus(String tenantId, String customerId, UpdateAmlStatusRequest request) {
        AmlCase existing = amlRepository.findByTenantIdAndCustomerId(tenantId, customerId)
                .orElseGet(() -> defaultCase(tenantId, customerId));

        if (request.suspiciousActivityReported() && isBlank(request.goamlReference())) {
            throw new IllegalArgumentException("goamlReference is required when suspiciousActivityReported is true");
        }
    ...
    public AmlStatusResponse assessForOrigination(String tenantId, String customerId, OriginationAssessmentRequest request) {
        AmlCase amlCase = amlRepository.findByTenantIdAndCustomerId(tenantId, customerId)
                .orElseGet(() -> defaultCase(tenantId, customerId));
    ...
    private Assessment assess(String tenantId, AmlCase amlCase, BigDecimal loanAmount) {
        ...
        if (amlCase.pepFlag() && amlCase.riskLevel() == AmlRiskLevel.HIGH && !amlCase.sourceOfFundsChecked()) {
            return new Assessment(true, false, "Enhanced due diligence required for high-risk PEP");
        }
        if (loanAmount != null && loanAmount.compareTo(new BigDecimal("2000.00")) >= 0 && !amlCase.sourceOfFundsChecked()) {
            return new Assessment(true, false, "Source of funds review required for higher-value origination");
        }
