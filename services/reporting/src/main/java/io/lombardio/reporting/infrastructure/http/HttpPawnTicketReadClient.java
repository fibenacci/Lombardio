...
public class HttpPawnTicketReadClient implements PawnTicketReadClient {
    ...
    public List<ReportedPawnTicketOverview> listTickets(String tenantId, String bearerToken) {
        PawnTicketOverviewProjection[] response = restClient.get()
                .uri(uriBuilder -> uriBuilder.path("/api/v1/tenants/{tenantId}/pawn-tickets").build(tenantId))
        ...
        return List.of(response).stream()
                .map(item -> new ReportedPawnTicketOverview(
                        item.ticketNumber(),
                        item.totalLoanValue(),
    ...
    public List<ReportedCashTransaction> listCashTransactions(String tenantId, String bearerToken) {
        CashTransactionProjection[] response = restClient.get()
                .uri(uriBuilder -> uriBuilder.path("/api/v1/tenants/{tenantId}/cash-transactions").build(tenantId))
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + bearerToken)
                .retrieve()
        ...
        return List.of(response).stream()
                .map(item -> new ReportedCashTransaction(
                        item.type(),
                        item.interestAmount(),
