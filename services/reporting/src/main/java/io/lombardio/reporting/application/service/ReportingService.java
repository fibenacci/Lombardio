...
public class ReportingService {
  ...
  public ReportingService(
      LoanReadClient loanReadClient, PawnTicketReadClient pawnTicketReadClient, Clock clock) {
  ...
  public ReportingDashboardResponse getDashboard(
      String tenantId, int rangeDays, String bearerToken) {
    ...
    List<PawnTicketReadClient.ReportedPawnTicketOverview> tickets =
        pawnTicketReadClient.listTickets(tenantId, bearerToken);
    List<PawnTicketReadClient.ReportedCashTransaction> cashTransactions =
        pawnTicketReadClient.listCashTransactions(tenantId, bearerToken);
    ...
    BigDecimal realizedRevenue =
        cashTransactions.stream()
            ...
            .map(
                transaction ->
                    zeroIfNull(transaction.interestAmount())
                        .add(zeroIfNull(transaction.operatingFeeAmount())))
            .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    BigDecimal averageTicketValue =
        activeTicketCount == 0
            ...
            : activeLoanExposure.divide(
                BigDecimal.valueOf(activeTicketCount), 2, RoundingMode.HALF_UP);
  ...
  private List<FinanceTrendPointResponse> buildFinanceTrend(
    ...
    for (PawnTicketReadClient.ReportedCashTransaction transaction : cashTransactions) {
      ...
      if (transactionDate == null
          || transactionDate.isBefore(rangeStart)
          || transactionDate.isAfter(rangeEnd)) {
        continue;
      }

      inflowByDay.computeIfPresent(
          transactionDate, (key, current) -> current.add(zeroIfNull(transaction.totalAmount())));
      revenueByDay.computeIfPresent(
          transactionDate,
          (key, current) ->
              current
                  .add(zeroIfNull(transaction.interestAmount()))
                  .add(zeroIfNull(transaction.operatingFeeAmount())));
    }
  ...
  private List<InventoryCategoryResponse> buildInventoryByCategory(
      List<LoanReadClient.ReportedLoanCase> loans) {
    ...
    for (LoanReadClient.ReportedLoanCase loan : loans) {
      for (LoanReadClient.ReportedLoanPosition position : loan.positions()) {
        ...
        InventoryAggregate aggregate =
            aggregates.computeIfAbsent(category, ignored -> new InventoryAggregate());
  ...
  private List<TransactionMixResponse> buildTransactionMix(
    ...
    for (PawnTicketReadClient.ReportedCashTransaction transaction : cashTransactions) {
      ...
      TransactionAggregate aggregate =
          aggregates.computeIfAbsent(type, ignored -> new TransactionAggregate());
