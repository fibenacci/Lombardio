...
public class OnlineAuctionPersistenceAdapter implements OnlineAuctionRepository {
  ...
  public OnlineAuction save(OnlineAuction auction) {
    ...
    entity.setRegistrations(
        auction.registrations().stream().map(item -> toRegistrationEntity(item, entity)).toList());
  ...
  private BidderRegistrationEntity toRegistrationEntity(
      BidderRegistration registration, OnlineAuctionEntity auction) {
