...
public class OnlineAuctionService {
    ...
    public OnlineAuctionService(OnlineAuctionRepository auctionRepository,
    ...
    private OnlineAuctionService(OnlineAuctionRepository auctionRepository,
                                 RealtimePublisher realtimePublisher,
                                 RealtimeSessionTokenService realtimeSessionTokenService,
    ...
    public OnlineAuctionResponse createAuction(String tenantId, CreateOnlineAuctionRequest request) {
        ...
        OnlineAuction auction = new OnlineAuction(
                auctionId,
                tenantId,
                request.title(),
    ...
    public OnlineAuctionResponse publishAuction(String tenantId, String auctionId) {
        ...
        OnlineAuction updated = new OnlineAuction(
                current.id(), current.tenantId(), current.title(), current.slug(), OnlineAuctionStatus.PUBLISHED,
                current.channelName(), current.minimumIncrement(), current.countdownSeconds(), Instant.now(), current.liveStartedAt(), current.countdownEndsAt(), current.closedAt(),
                current.createdAt(), Instant.now(), current.lots(), current.registrations()
        );
    ...
    public OnlineAuctionResponse startAuction(String tenantId, String auctionId) {
        ...
        Instant now = Instant.now();
        OnlineAuction updated = new OnlineAuction(
                current.id(), current.tenantId(), current.title(), current.slug(), OnlineAuctionStatus.LIVE,
                current.channelName(), current.minimumIncrement(), current.countdownSeconds(), current.publishedAt(), now, now.plusSeconds(current.countdownSeconds()), current.closedAt(),
                current.createdAt(), Instant.now(), current.lots(), current.registrations()
        );
        OnlineAuction saved = auctionRepository.save(updated);
    ...
    public OnlineAuctionResponse closeAuction(String tenantId, String auctionId) {
        ...
        OnlineAuction updated = new OnlineAuction(
                current.id(), current.tenantId(), current.title(), current.slug(), OnlineAuctionStatus.CLOSED,
                current.channelName(), current.minimumIncrement(), current.countdownSeconds(), current.publishedAt(), current.liveStartedAt(), current.countdownEndsAt(), Instant.now(),
                current.createdAt(), Instant.now(), current.lots(), current.registrations()
        );
        OnlineAuction saved = auctionRepository.save(updated);
    ...
    public BidderRegistrationResponse registerBidder(String tenantId, String auctionId, RegisterBidderRequest request) {
        OnlineAuction current = requirePublicAuction(tenantId, auctionId);
        BidderRegistration registration = new BidderRegistration(
                "obr-" + UUID.randomUUID(),
                request.displayName(),
                request.email(),
                request.legalName(),
        ...
        OnlineAuction updated = new OnlineAuction(
                current.id(), current.tenantId(), current.title(), current.slug(), current.status(),
                current.channelName(), current.minimumIncrement(), current.countdownSeconds(), current.publishedAt(), current.liveStartedAt(), current.countdownEndsAt(), current.closedAt(),
                current.createdAt(), Instant.now(), current.lots(), registrations
        );
        OnlineAuction saved = auctionRepository.save(updated);
    ...
    public OnlineAuctionResponse reviewRegistration(String tenantId, String auctionId, String registrationId, BidderReviewRequest request) {
        OnlineAuction current = requireAuction(tenantId, auctionId);
        ...
        BidderApprovalStatus status = switch (request.decision().trim().toUpperCase()) {
        ...
        if (status == BidderApprovalStatus.APPROVED
                && (kycStatus != ReviewCheckStatus.PASSED || accountCheckStatus != ReviewCheckStatus.PASSED)) {
        ...
        List<BidderRegistration> registrations = current.registrations().stream()
                .map(item -> item.id().equals(registrationId)
    ...
    public OnlineAuctionResponse placeBid(String tenantId, String auctionId, PlaceOnlineBidRequest request) {
        OnlineAuction current = requirePublicAuction(tenantId, auctionId);
        if (current.status() != OnlineAuctionStatus.LIVE) {
        ...
        BidderRegistration bidder = current.registrations().stream()
        ...
        List<OnlineAuctionLot> updatedLots = current.lots().stream()
                .map(lot -> updateLotBid(current, lot, request, bidder))
                .toList();
        OnlineAuction updated = new OnlineAuction(
                current.id(), current.tenantId(), current.title(), current.slug(), current.status(),
                current.channelName(), current.minimumIncrement(), current.countdownSeconds(), current.publishedAt(), current.liveStartedAt(), current.countdownEndsAt(), current.closedAt(),
                current.createdAt(), Instant.now(), updatedLots, current.registrations()
        );
        OnlineAuction saved = auctionRepository.save(updated);
    ...
    public RealtimeSessionResponse issueRealtimeSession(String tenantId, String auctionId, String accessToken) {
        OnlineAuction auction = requirePublicAuction(tenantId, auctionId);
        BidderRegistration bidder = auction.registrations().stream()
        ...
        if (bidder.kycStatus() != ReviewCheckStatus.PASSED || bidder.accountCheckStatus() != ReviewCheckStatus.PASSED) {
            throw new IllegalArgumentException("Bidder compliance checks are incomplete");
        }
        RealtimeSession session = realtimeSessionTokenService.createSession(bidder.paddleNumber(), auction.channelName());
        return new RealtimeSessionResponse(session.wsUrl(), session.channel(), session.connectionToken(), session.subscriptionToken());
    }

    private OnlineAuctionLot updateLotBid(OnlineAuction auction, OnlineAuctionLot lot, PlaceOnlineBidRequest request, BidderRegistration bidder) {
    ...
    private OnlineAuction requireAuction(String tenantId, String auctionId) {
        return auctionRepository.findByTenantIdAndId(tenantId, auctionId)
    ...
    private OnlineAuction requirePublicAuction(String tenantId, String auctionId) {
        return auctionRepository.findPublicByTenantIdAndId(tenantId, auctionId)
    ...
    private ReviewCheckStatus parseCheckStatus(String value) {
        return switch (value.trim().toUpperCase()) {
            ...
            default -> throw new IllegalArgumentException("Check status must be PENDING, PASSED or FAILED");
        };
    }
    ...
    private OnlineAuctionLotResponse toLotResponse(OnlineAuctionLot lot) {
        return new OnlineAuctionLotResponse(
                lot.id(), lot.lotNumber(), lot.title(), lot.description(), lot.startingBid(), lot.currentBid(), lot.highestBidderAlias()
        );
    }
