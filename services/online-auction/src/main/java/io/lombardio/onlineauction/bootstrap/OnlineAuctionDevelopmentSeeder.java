...
public class OnlineAuctionDevelopmentSeeder {
    ...
    private OnlineAuction buildAuction(DemoTenant tenant, int tenantIndex, int auctionIndex) {
        OnlineAuctionStatus status = switch (auctionIndex % 4) {
        ...
        Instant createdAt = Instant.now().minusSeconds((long) (auctionIndex + tenantIndex * 12) * 86_400L);
        Instant publishedAt = status == OnlineAuctionStatus.DRAFT ? null : createdAt.plusSeconds(43_200);
        Instant liveStartedAt = status == OnlineAuctionStatus.LIVE || status == OnlineAuctionStatus.CLOSED ? createdAt.plusSeconds(86_400) : null;
        Instant countdownEndsAt = status == OnlineAuctionStatus.LIVE ? Instant.now().plusSeconds(900 + auctionIndex * 60L) : null;
        Instant closedAt = status == OnlineAuctionStatus.CLOSED ? liveStartedAt.plusSeconds(21_600) : null;
    ...
    private List<OnlineAuctionLot> buildLots(DemoTenant tenant, int auctionIndex, OnlineAuctionStatus status) {
        ...
        for (int lotIndex = 1; lotIndex <= lotCount; lotIndex++) {
            ...
            BigDecimal currentBid = switch (status) {
            ...
            lots.add(new OnlineAuctionLot(
                    "online-lot-" + tenant.key() + "-" + String.format("%03d", auctionIndex) + "-" + String.format("%02d", lotIndex),
                    lotIndex,
                    lotIndex % 2 == 0 ? "Designerschmuck Los " + lotIndex : "Elektronik Los " + lotIndex,
                    lotIndex % 2 == 0 ? "Ring, Kette und Armband im Set" : "Smartphone, Tablet oder Kamera im Bieterpaket",
                    startingBid,
                    currentBid,
                    status == OnlineAuctionStatus.DRAFT ? null : "B" + String.format("%03d", 100 + lotIndex)
            ));
        }
        return lots;
    }
    ...
    private List<BidderRegistration> buildRegistrations(DemoTenant tenant, int auctionIndex) {
        ...
        for (int index = 1; index <= registrationCount; index++) {
            BidderApprovalStatus approvalStatus = switch (index % 5) {
                case 0 -> BidderApprovalStatus.REJECTED;
                case 1, 3 -> BidderApprovalStatus.APPROVED;
                default -> BidderApprovalStatus.PENDING;
            };
            ReviewCheckStatus reviewStatus = approvalStatus == BidderApprovalStatus.REJECTED ? ReviewCheckStatus.FAILED :
                    approvalStatus == BidderApprovalStatus.APPROVED ? ReviewCheckStatus.PASSED : ReviewCheckStatus.PENDING;

            registrations.add(new BidderRegistration(
                    "bidder-" + tenant.key() + "-" + String.format("%03d", auctionIndex) + "-" + String.format("%02d", index),
                    "Bieter " + index,
                    ...
                    tenant.key().substring(0, Math.min(3, tenant.key().length())).toUpperCase() + String.format("%03d", auctionIndex * 10 + index),
                    "token-" + tenant.key() + "-" + auctionIndex + "-" + index,
                    approvalStatus,
                    reviewStatus,
                    approvalStatus == BidderApprovalStatus.REJECTED ? ReviewCheckStatus.FAILED : ReviewCheckStatus.PASSED,
                    approvalStatus == BidderApprovalStatus.REJECTED ? "Adressnachweis unvollstaendig" : null,
                    approvalStatus == BidderApprovalStatus.APPROVED ? Instant.now().minusSeconds(index * 1_800L) : null,
