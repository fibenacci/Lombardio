...
@Component
public class KycAuthenticationFilter extends OncePerRequestFilter {

    private final IdentityAccessClient identityAccessClient;

    public KycAuthenticationFilter(IdentityAccessClient identityAccessClient) {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (authorization != null && authorization.startsWith("Bearer ")) {
            ...
            identityAccessClient.currentUser(tokenValue).ifPresent(currentUser -> {
                AuthenticatedKycUser principal = new AuthenticatedKycUser(
                        currentUser.id(),
