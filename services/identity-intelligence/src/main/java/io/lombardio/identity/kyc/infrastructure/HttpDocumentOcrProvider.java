...
/**
 * Advanced implementation of DocumentOcrProvider using Regula Forensics Web API.
 * Patterns and models aligned with the official JS Client and OpenAPI.
 * Supports Visual, MRZ, Barcode and Graphics results.
 */
@Component
public class HttpDocumentOcrProvider implements DocumentOcrProvider {
    ...
    public Optional<DocumentOcrResult> prefill(String tenantId, String frontImageDataUrl, String backImageDataUrl) {
        try {
            ...
            log.info("[OCR] Received response from Regula with {} containers", response.ContainerList().List().size());
    ...
    private ProcessRequest buildRequest(String frontUrl, String backUrl) {
        ...
        return new ProcessRequest(
            new ProcessParams("FullProcess", true),
            images
        );
    }
    ...
    private Optional<DocumentOcrResult> mapResponse(ProcessResponse response) {
        ...
        for (ResultItem item : response.ContainerList().List()) {
            if (item.Text() != null && item.Text().fieldList() != null) {
                log.info("[OCR] Parsing text container type: {} with {} fields", item.result_type(), item.Text().fieldList().size());
            ...
            if (item.Graphics() != null && item.Graphics().fieldList() != null) {
                log.info("[OCR] Parsing graphics container type: {} with {} fields", item.result_type(), item.Graphics().fieldList().size());
    ...
    private LocalDate parseDate(String dateStr) {
        ...
        List<String> formats = List.of("dd.MM.yyyy", "yyyy-MM-dd", "MM/dd/yyyy", "dd-MM-yyyy", "yyMMdd");
