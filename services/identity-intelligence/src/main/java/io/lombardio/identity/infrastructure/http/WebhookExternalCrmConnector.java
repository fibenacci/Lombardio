...
public class WebhookExternalCrmConnector implements ExternalCrmConnector {
  ...
  public WebhookExternalCrmConnector(
      RestClient.Builder restClientBuilder, @Value("${external-crm.base-url:}") String crmBaseUrl) {
  ...
  public List<Customer> search(String tenantId, String query) {
    ...
    try {
          ...
          .map(
              crmCustomer ->
                  new Customer(
                      ...
                      null, // BirthDate parsing if provided by
                      // CRM
                      crmCustomer.phone(),
                      crmCustomer.email(),
                      false,
      ...
      System.err.println(
          "[ERROR] Failed to query external CRM for tenant " + tenantId + ": " + e.getMessage());
      return Collections.emptyList();
    }
  }
