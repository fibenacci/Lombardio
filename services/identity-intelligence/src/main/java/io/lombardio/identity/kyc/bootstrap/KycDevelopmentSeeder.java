 ...
 */
package io.lombardio.identity.kyc.bootstrap;
...
public class KycDevelopmentSeeder {
  ...
  KycDevelopmentSeeder(KycRepository kycRepository, DemoDataProperties demoDataProperties) {
  ...
  private KycRecord buildRecord(DemoTenant tenant, int tenantIndex, int customerIndex) {
    KycStatus status =
        switch (customerIndex % 8) {
          case 0, 4, 5 -> KycStatus.APPROVED;
          case 1, 6 -> KycStatus.IN_PROGRESS;
          case 2 -> KycStatus.REJECTED;
          default -> KycStatus.NOT_STARTED;
        };
    KycVerificationMode mode =
        customerIndex % 3 == 0 ? KycVerificationMode.PROVIDER : KycVerificationMode.MANUAL;
    ...
    return new KycRecord(
        ...
        mode == KycVerificationMode.PROVIDER
            ? "DEMO-" + tenant.key() + "-" + String.format("%05d", customerIndex)
            : null,
  ...
  private String decisionNote(KycStatus status, KycVerificationMode mode, int customerIndex) {
    return switch (status) {
      case APPROVED ->
          mode == KycVerificationMode.PROVIDER
              ? "Automatisierte Identifizierung erfolgreich abgeschlossen"
              : "Manuelle Identitaetspruefung freigegeben";
      case IN_PROGRESS -> "Dokumentenpruefung und Adressabgleich noch offen";
      case REJECTED ->
          customerIndex % 2 == 0 ? "Dokument nicht mehr gueltig" : "Abweichung bei Personenangaben";
