...
public class LoanOriginationService {
    ...
    public List<LoanCase> listLoans(String tenantId, String customerId) {
        List<LoanCase> loans = customerId == null || customerId.isBlank()
    ...
    public LoanCase createLoan(String tenantId, CreateLoanCommand request) {
        try {
            ...
            if (request.thirdPartyPledgorPresentation()) {
                ...
                if (request.powerOfAttorneyDocumentDataUrl() == null || request.powerOfAttorneyDocumentDataUrl().isBlank()) {
                    throw new IllegalArgumentException("powerOfAttorneyDocumentDataUrl is required for third-party presentation");
                }
            }
            ...
            List<LoanPosition> positions = request.positions().stream()
                    .map(position -> buildPosition(tenantId, position))
                    .toList();
            BigDecimal totalLoanValue = positions.stream()
                    .map(LoanPosition::pledgedValue)
                    .reduce(BigDecimal.ZERO, BigDecimal::add);
            AmlDirectory.AmlAssessment amlAssessment = amlDirectory.assessForOrigination(
                    tenantId,
                    request.customerId(),
                    totalLoanValue
            );
            ...
            List<PawnTicket> pawnTickets = positions.stream()
    ...
    private PawnTicket issuePawnTicket(String tenantId, CustomerProfile customer, List<LoanPosition> ticketPositions, CreateLoanCommand request) {
        BigDecimal totalLoanValue = ticketPositions.stream()
                .map(LoanPosition::pledgedValue)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    private LoanPosition buildPosition(String tenantId, CreateLoanPositionCommand request) {
        ValuationGuideline guideline = valuationGuidelineRepository.findById(request.guidelineId())
                .filter(item -> item.tenantId().equals(tenantId))
                .orElseThrow(() -> new IllegalArgumentException("Valuation guideline not found"));

        BigDecimal pledgedValue = request.pledgedValue() != null ? request.pledgedValue() : guideline.baseLoanValue();
    ...
    private PledgeRecord buildPledgeRecord(String loanCaseId, String tenantId, CustomerProfile customer, CreateLoanCommand request) {
