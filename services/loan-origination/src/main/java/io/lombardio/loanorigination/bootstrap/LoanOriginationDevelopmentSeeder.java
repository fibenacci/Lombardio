...
public class LoanOriginationDevelopmentSeeder {
  ...
  private LoanCase buildLoanCase(
      ReferenceDataSeeder.DemoTenant tenant,
      int tenantIndex,
      int caseIndex,
      List<ValuationGuideline> guidelines) {
    ...
    for (int positionIndex = 0; positionIndex < positionCount; positionIndex++) {
      ValuationGuideline guideline =
          guidelines.get((caseIndex + positionIndex) % guidelines.size());
      BigDecimal baseLoanValue =
          guideline.baseLoanValue().add(BigDecimal.valueOf(positionIndex * 35L));
    ...
    BigDecimal totalInterestAmount =
        totalLoanValue.multiply(monthlyInterestRate).setScale(2, java.math.RoundingMode.HALF_UP);
    ...
    return new LoanCase(
        ...
        new CustomerProfile(
            ...
            firstName + " " + lastName,
            LocalDate.of(
                1965 + ((customerIndex + tenantIndex) % 35),
                ((customerIndex - 1) % 12) + 1,
                ((customerIndex - 1) % 27) + 1),
            "+49 1"
                + String.format("%02d", 50 + tenantIndex)
                + " "
            ...
            customerIndex % 8 == 2
                ? "REJECTED"
                : (customerIndex % 3 == 0 ? "IN_PROGRESS" : "APPROVED"),
            customerIndex % 8 != 2,
